    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register([Bind(Include = "ma_kh,mat_khau,ho_ten,cmt,sdt,mail")] tblKhachHang tblKhachHang)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    tblKhachHang.mat_khau = GetMD5(tblKhachHang.mat_khau);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Login(string ma_kh, string mat_khau)
        {
            if (ModelState.IsValid)
            {
                var f_password = GetMD5(mat_khau);
                //var obj = db.tblKhachHangs.Where(a => a.ma_kh.Equals(objUser.ma_kh) && a.mat_khau.Equals(f_password)).FirstOrDefault();
                var data = db.tblKhachHangs.Where(s => s.ma_kh.Equals(ma_kh) && s.mat_khau.Equals(f_password)).ToList();
                tblKhachHang kh = db.tblKhachHangs.SingleOrDefault(a => a.ma_kh.Equals(ma_kh) && a.mat_khau.Equals(f_password));
                if (kh != null)
                {
                    Session["KH"] = kh;
                    return RedirectToAction("BookRoom", "Home");
